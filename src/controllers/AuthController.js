...
// Mã secretKey này phải được bảo mật tuyệt đối, các bạn có thể lưu vào biến môi trường hoặc file
const refreshTokenSecret = process.env.REFRESH_TOKEN_SECRET || "access-token-secret-example-nmquang";
/**
 * controller login
 * @param {*} req 
 * @param {*} res 
...
let login = async (req, res) => {
    try {
        console.log(`Đang giả lập hành động đăng nhập thành công với Email: ${req.body.email} và Password: ${req.body.password}`);
 ...
 */
 let verifyUser = async (req, res) => {
    if (req.query.confirmationCode) {
       var result =await  axios.get(`http://localhost:8017/verifyUser?confirmationCode=${req.query.confirmationCode}`);
       if(result.data && result.data.message === 'success'){
        res.redirect('login?verify_status=1');
       }
       else{
        res.redirect('login');
       }
